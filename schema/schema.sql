`                    create extension if not exists "uuid-ossp";
                    ...
                    create or replace function public.trg_card_assignees_activity()
                    returns trigger
                    security definer
                    set search_path = public
                    language plpgsql as $$
                    ...
                    begin
                      if TG_OP = 'INSERT' then
                        v_card := new.card_id;
                        perform public.log_activity(v_card, 'assignee.add', jsonb_build_object('userId', new.user_id));
                        return new;
                      elsif TG_OP = 'DELETE' then
                        v_card := old.card_id;
                        perform public.log_activity(v_card, 'assignee.remove', jsonb_build_object('userId', old.user_id));
                        return old;
                      end if;
                      return null;
                    end;
                    ...
                    drop trigger if exists trg_card_assignees_activity_ins on card_assignees;
                    create trigger trg_card_assignees_activity_ins
                    after insert on card_assignees
                    for each row execute function public.trg_card_assignees_activity();
                    drop trigger if exists trg_card_assignees_activity_del on card_assignees;
                    create trigger trg_card_assignees_activity_del
                    after delete on card_assignees
                    for each row execute function public.trg_card_assignees_activity();

                    -- RPC to list all members of a workspace (security definer with membership check)
                    create or replace function public.list_workspace_members(p_ws_id uuid)
                    returns table(user_id uuid, role text)
                    security definer
                    set search_path = public
                    language sql stable as $$
                      -- Only return rows if the caller is a member of the workspace
                      select m.user_id, m.role
                      from workspace_members m
                      where m.workspace_id = p_ws_id
                        and is_workspace_member(auth.uid(), p_ws_id);
                    $$;

                    -- Allow clients to execute the RPC
                    grant execute on function public.list_workspace_members(uuid) to anon, authenticated;
`